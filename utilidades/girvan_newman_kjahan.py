...
def runGirvanNewman(G, Orig_deg, m_):
    ...
    while True:
        ...
        if Q > BestQ:
            ...
            #return Bestcomps #uma unica iteracao
...
#funcao para passar o grafo por parametro
def executar_Girvan_Newman_Kjahan(G):
    n = G.number_of_nodes()  # numero de vertices
    A = nx.adj_matrix(G)  # matriz de adjacencias

    m_ = 0.0  # soma dos pesos de todas as arestas do grafo, dividido por 2
    for i in range(0, n):
        for j in range(0, n):
            m_ += A[i, j]
    m_ = m_ / 2.0
