# -*- coding: utf-8 -*-
__author__ = 'Márcio Vinícius Dias, from https://github.com/kjahan/community'
...
def runGirvanNewman(G, Orig_deg, m_):
    ...
    while True:
        ...
        if Q > BestQ:
            BestQ = Q
            Bestcomps = sorted(nx.connected_components(G), key=len, reverse=True)    #Best Split
            #print "comps:"
            print Bestcomps
    ...
    if BestQ > 0.0:
        ...
        print Bestcomps
        result = [BestQ, Bestcomps]
    else:
        result = [BestQ, [G.nodes()]]
        print "Best Q: %f" % BestQ
...
def main(G):
    ...
    #graph_fn = file_name = "dataset/karate.txt"
    ...
    #run Newman alg
    result1 = runGirvanNewman(G, Orig_deg, m_)
