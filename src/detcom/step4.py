...
class Etp4():
    ...
    def run(self, fileWithTreatedRecords, fileWithOriginalRecords, categorical, isHomogeneousNetwork):
        ...
        print "Algorithm selected to clustering records was {}".format( algorithm[algorithmChoose][0] )

        if algorithmChoose == 0:
         #   import arquivofitx
          #  arquivofitx.distanceForText(textVet)
        ...
        log = open("detcomNV/output/fim.txt", "w")
        log.write("camila lindona")
        log.close()
